$(function(){
	...
	$('.commit-author').on('click',function(){
		config.formSubmit('#authorForm',config.ajaxAddress.specialSubject.updateSort,function(data){
			if(data.code==200){
	...
	layui.use('laytpl',function(){
		laytpl = layui.laytpl;
		config.ajax('get',config.ajaxAddress.specialSubject.updateSort,function(data){
		
			var tempHtml=editSortPic.innerHTML;
			laytpl(tempHtml).render(data.data,function(html){
