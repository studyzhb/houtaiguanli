require(['jquery','main','ajaxAddress','lay-model','log'],function($,myObj,ajaxAddress,layObj,log){
    ...
    var navObj={
        ...
        getSingleInfo:function(id) {
            ...
            var singleInfo;
            $.each(navObj.data.arrObligation,function(index,item){
                if(id==item.id){
                    singleInfo=item;
                    return false;
                }
    ...
    $('#tableList').on('click','.editNavInfo',function(){
        ...
        if(navObj.isUpOrDownClick){
            navObj.isUpOrDownClick=false;
            var sItem=navObj.getSingleInfo(navId);
        ...
        form.verify({
		  ...
		  form.on('submit(formEditDemo)', function(data1){
		    /*layer.alert(JSON.stringify(data.field), {
		  	...
		  	common.tools.ajax('post',ajaxAddress.preFix+ajaxAddress.obligation.update,function(data){
