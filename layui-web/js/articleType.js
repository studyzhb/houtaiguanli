require(['jquery','main','ajaxAddress','lay-model','log'],function($,myObj,ajaxAddress,layObj,log){
    ...
    var classObj={
        ...
        methods:{
            ...
            updatePageNum:function(num){
                ...
                common.tools.ajax('get',ajaxAddress.preFix+ajaxAddress.articleType.showlist,function(data){
                    log.d(data);
                    if(data.code==200){
            ...
            updateStatusType:function(sta,upId,obj){
                ...
                // item.status=sta;
                common.tools.ajax('post',ajaxAddress.preFix+ajaxAddress.articleType.updateStatus,function(data){
                    log.d(data);
    ...
    setTimeout(function(){
        ...
        form.on('submit(shopInfo)',function(formParams){
            ...
            layer.load();
            common.tools.ajax('post',ajaxAddress.preFix+ajaxAddress.articleType.addType,function(data){
                    log.d(data);
                    if(data.code==200){
        ...
        form.on('submit(editorAreaType)',function(formParams){
            log.d(formParams.field)
            layObj.layer.load();
            common.tools.ajax('post',ajaxAddress.preFix+ajaxAddress.articleType.editType,function(data){
                    log.d(data);
                    if(data.code==200){
                        layer.msg('更新成功');
                         layObj.layer.closeAll();
    ...
    //删除文章类型
     $('#all-sort-list').on('click','.deleteArticleType',function(){
         var id=$(this).data('id');
        layObj.layer.confirm('确定执行删除操作？',function(index){
            layObj.layer.close(index);
            classObj.methods.deleteArticleType(id);
        })
