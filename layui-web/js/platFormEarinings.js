require(['jquery','main','ajaxAddress','lay-model','log'],function($,myObj,ajaxAddress,layObj,log){
    ...
    var classObj={
        ...
        methods:{
            ...
            updatePageNum:function(num){
                ...
                common.tools.ajax('get',ajaxAddress.obligationPreFix+ajaxAddress.platForm.showlist,function(data){
                    log.d(data);
                    if(data.code==200){
                        classObj.data.arrData=data.data.pay_list;
                        
                        classObj.data.pageCount=Math.ceil(data.data.all_num/10);
                        $('.detailCount').text(data.data.all_num);
                        classObj.methods.updateArealist(data.data.pay_list);
                        if(fistLoad){
     ...
     */
     $('.statusTabMoney').on('click',function(){
            ...
            var status=$(this).data('status');
            fistLoad=true;
            classObj.data.cacheData={type:status};
            classObj.methods.updatePageNum(classObj.data.currentPageNum);
     })
