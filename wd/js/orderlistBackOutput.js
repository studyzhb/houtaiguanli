...
$('#purchaselist').on('click','.lookorderInfo',function(){
	config.ajax('get',config.ajaxAddress.orderlistBackCheckCount,function(data){
...
$('.checkConfirm').on('click',function(){
	...
	config.ajax('post',config.ajaxAddress.orderlistBackCommit,function(data){
		console.log(data);
		if(data.code==200){
			layer.closeAll();
            layer.msg('操作成功');
...
function confirmIntoStock(){
	config.formSubmit('#confirmInto',config.ajaxAddress.orderlistBackOutput,function(data){
		console.log(data);
		if(data.code==200){
                layer.msg('操作成功');
                setTimeout(function(){
