require(['jquery','main','ajaxAddress','lay-model','log','params'],function($,myObj,ajaxAddress,layObj,log,params){
    ...
    var ShopObj={
        ...
        methods:{
            updateStatus:function(sta){
                $('.statusTab').each(function(){
                    if($(this).data('status')==sta){
                        $(this).addClass('layui-this').siblings().removeClass('layui-this');
                        return false;
                    }
                })
            },
            updateRequestUrl:function(sta){
                ShopObj.methods.updateStatus(sta);
                //sta 0:完结 1 需要审核提现申请的 2 未打款的提现申请 3 财务拒绝的申请

                switch (sta) {
                    case 0:
                        ShopObj.data.requestUrl=ajaxAddress.commercialManager.withdraw.historyList;
                        break;
                    case 1:
                        ShopObj.data.requestUrl=ajaxAddress.commercialManager.withdraw.willWithdrawList;
                        break;
                    case 2:
                        ShopObj.data.requestUrl=ajaxAddress.commercialManager.withdraw.notWithdrawList;
                        break;
                    case 3:
                        ShopObj.data.requestUrl=ajaxAddress.commercialManager.withdraw.refuWithdrawList;
                        break;
                    default:
                        ShopObj.data.requestUrl=ajaxAddress.commercialManager.withdraw.historyList;
                        break;
            ...
            //审核或者打款
            updateWithdrawStatus:function(para){
                para=para||{};
                common.tools.ajax('post',ajaxAddress.preFix+ajaxAddress.commercialManager.withdraw.changeWithStatus,function(data){
                    if(data.code==200){
    ...
    $('.statusTab').on('click',function(){
        ...
        ShopObj.data.currentTabPage=sta-0;
        // $(this).addClass('layui-this').siblings().removeClass('layui-this');
   ...
   setTimeout(function(){
       log.d(layObj);
        ...
        //通过状态和名字搜索列表
        form.on('submit(searchOrderList)',function(paraData){
            
            ShopObj.methods.updatePageNum(ShopObj.data.currentPage,paraData.field);
        })
        //
        form.on('submit(confirmMoney)',function(paraData){
            layObj.layer.load();
            ShopObj.methods.updateWithdrawStatus(paraData.field);
