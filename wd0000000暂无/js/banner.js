...
$('.fullname').text(unescape(fname));
    $('#confirmsavetext').on('click',function(){
        ...
        config.ajax('get',config.ajaxAddress.bannerImg,function(data){
          ...
          if(data.info){
            ...
            var arr=eval(data.info)?eval(data.info):'';
            if(arr instanceof Array){
              $('.image-suolve').html('');
              ImageWrapper.suolveImg=arr;
              $.each(arr,function(index,item){
                
                $('.image-suolve').append(config.formatTemplate({imgsrc:item.pic,selectIndex:index},$('#image-suolve').html()));
              });
            }
